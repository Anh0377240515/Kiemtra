...
function mostCommonly(str) {
    var charCount={};
    //xem số lần lặp kí tự
    for (var i=0;i<str.length;i++) {
        var char=str[i];
        charCount[char]=(charCount[char]||0)+1;
    }
    //khởi tạo biến lưu đối tượng shnn,slsh
    var maxChar='';
    var maxCharCount=0;
    //lặp tìm kí tự nhiều nhất
...
function areAnagrams(str1, str2) {
    //loại bỏ khoảng trắng và chuyển về chữ thường,tách,xếp,gộp
    var sortedStr1 = str1.replace(/\s/g, '').toLowerCase().split('').sort().join('');
    var sortedStr2 = str2.replace(/\s/g, '').toLowerCase().split('').sort().join('');
    //so sánh
...
function isPalindrome(str) {
    ...
    //so sánh chuỗi ban đầu với chuỗi đã đảo ngược
...
function isBalanced(str) {
    //Khởi tạo 1 biến lưu dấu ngoặc mở
    var stack = [];
    //Khởi tạo 1 oj lưu cặp ngoặc
    var brackets = {
        "(": ")",
        "[": "]",
        "{": "}"
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (brackets[char]) {
            ...
            // check xem stack có trống không và nếu ký tự hiện tại tương ứng với dấu ngoặc mở của ký tự cuối cùng trong stack
            if (stack.length===0||brackets[stack.pop()]!==char) {
                return false; 
        }
        }
    }
    //nếu trống thì cân bằng
...
function ArmstrongNumbers(number) {
    ...
    //lấy số chữ số 
    var digits = strNumber.length;
    //khởi tạo sum =0
    var sum = 0;
    //lặp từng chữ
    for (var i = 0; i < digits; i++) {
        //chuyển thành số nguyên và tích luỹ thừa
        sum += Math.pow(parseInt(strNumber[i]), digits);
    }
    //check xem tổng voà có bằng tổng ra không
    return sum === number;
}
...
function sortObjectsByPopulation(objects) {
    //dùng sort() sắp xếp mảng
    objects.sort(function(a, b) {
        //dùng thuộc tính population so sánh 
